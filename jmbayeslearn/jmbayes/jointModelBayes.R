jointModelBayes <- function (lmeObject, survObject, timeVar,
    ...
    isResponseNormalDistributed <- FALSE
    areRENormalDistributed <- FALSE
    if (densLongCheck <- is.null(densLong)) {
        if (is.null(lmeObject$family)) {
            ...
            isResponseNormalDistributed <- TRUE
        } else {
    ...
    noDefaultTransFunc <- FALSE
    # transormation functions
    if (is.null(transFun)) {
        noDefaultTransFunc <- TRUE
    ...
    Funs <- list(transFun.value = transFun.value, transFun.extra = transFun.extra,
                 ...
                 noDefaultTransFunc=noDefaultTransFunc, isResponseNormalDistributed = isResponseNormalDistributed,
                 areRENormalDistributed = areRENormalDistributed)
    # control values
